...
exports.kiryuuManga = async function kiryuu(id, manga, quoted) {
  let txt = "Maaf User!\n";
  txt += "Harap Masukkan Manga yang ingin dicari!\n\n";
  txt += "Contoh: \n";
  txt += "/search kiryuu&naruto";
  if (!manga) return conn.reply(id, conn.monospace(txt), quoted);
  
  try {
    await conn.reply(id, conn.mess.wait, quoted);
    let result = await conn.axios("https://api.dhnjing.xyz/anime/kiryuu/search?manga=" + manga, "GET");
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      txt += `+ Manga: ${data[i].manga_name.replace(/[\n|\t]/g,"")}\n`;
