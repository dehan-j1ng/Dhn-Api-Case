...
exports.komikfoxManga = async function komikfox(id, manga, quoted) {
  let txt = "Maaf User!\n";
  txt += "Harap Masukkan Manga yang ingin dicari!\n\n";
  txt += "Contoh: \n";
  txt += "/search komikfox&naruto";
  if (!manga) return conn.reply(id, conn.monospace(txt), quoted);
  
  try {
    await conn.reply(id, conn.mess.wait, quoted);
    let result = await conn.axios("https://api.dhnjing.xyz/anime/komikfox/search?manga=" + manga, "GET");
